...
def update_input_frame(viewer):
    ...
    if viewer.creation_mode.get() == "coordonnees":
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT Nom FROM elements")
...
def create_point_from_click(viewer):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO elements (Nom, Lat, Lon) VALUES (?, ?, ?)",
        (nom, lat, lon)
    )
    conn.commit()
    conn.close()
...
def create_point_from_radial(viewer):
    ...
    conn = sqlite3.connect("point.db")
    cursor = conn.cursor()
    cursor.execute("SELECT Lat, Lon FROM elements WHERE Nom = ?", (point_depart_nom,))
...
def load_points(viewer):
    ...
    conn = sqlite3.connect("point.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, Nom, Lat, Lon FROM elements")
